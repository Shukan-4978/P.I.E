                ...
                return (
                  <div key={conv._id} onClick={() => selectConversation(conv)} className={`conv-item ${isActive ? 'active' : ''}`} style={{ padding: '1rem', display: 'flex', gap: '1rem', alignItems: 'center', cursor: 'pointer' }}>
                    <div style={{ position: 'relative', flexShrink: 0 }}>
                      <img src={other?.avatar ? `${other.avatar}` : `https://ui-avatars.com/api/?name=${encodeURIComponent(other?.name || 'U')}&background=6366f1&color=fff&size=48`} style={{ width: '48px', height: '48px', borderRadius: '16px', objectFit: 'cover', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }} alt="" />
            ...
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, padding: '1.25rem 2rem', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.02)', backdropFilter: 'blur(16px)', zIndex: 10, boxShadow: '0 4px 30px rgba(0,0,0,0.03)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
                ...
                <div style={{ position: 'relative' }}>
                  <img src={other?.avatar ? `${other.avatar}` : `https://ui-avatars.com/api/?name=${encodeURIComponent(other?.name || 'U')}&background=6366f1&color=fff&size=48`} style={{ width: '48px', height: '48px', borderRadius: '16px', objectFit: 'cover' }} alt="" />
              ...
              {messages.map((msg, i) => {
                ...
                return (
                  <div key={msg._id || i} className="msg-row" style={{ display: 'flex', justifyContent: isMe ? 'flex-end' : 'flex-start', gap: '1rem', alignItems: 'flex-end', transition: '0.2s', background: isSelected ? 'rgba(244,63,94,0.05)' : 'transparent', padding: isSelected ? '8px' : '0', borderRadius: '16px' }}>
                    {!isMe && <img src={other?.avatar ? `${other.avatar}` : `https://ui-avatars.com/api/?name=${encodeURIComponent(other?.name || 'U')}&background=6366f1&color=fff`} style={{ width: '36px', height: '36px', borderRadius: '12px', marginBottom: '4px', objectFit: 'cover' }} alt="" />}
                      ...
                      <AnimatePresence>
                        ...
                        {msg.type === 'image' && (
                          <div style={{ borderRadius: '20px', overflow: 'hidden', border: '1px solid var(--border)', background: 'var(--bg-secondary)', boxShadow: '0 8px 24px rgba(0,0,0,0.05)' }}>
                            <img src={`${msg.attachments?.[0]?.url}`} style={{ maxWidth: '100%', maxHeight: '350px', display: 'block', cursor: 'pointer' }} alt="Attachment" onClick={() => window.open(`${msg.attachments?.[0]?.url}`)} />
                          </div>
                        )}
                        {msg.type === 'file' && (
                          <div style={{ padding: '1rem', borderRadius: '20px', background: 'var(--bg-secondary)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '1rem', boxShadow: '0 8px 24px rgba(0,0,0,0.05)' }}>
                            ...
                            <a href={`${msg.attachments?.[0]?.url}`} download className="btn-ghost" style={{ padding: '10px', borderRadius: '12px' }}><Download size={20} /></a>
                          </div>
                        )}
                        {msg.type === 'offer' && (
