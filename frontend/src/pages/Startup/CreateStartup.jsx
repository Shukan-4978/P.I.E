...
import { Rocket, ChevronRight, ChevronLeft, Plus, X, Check, FileText, Image as ImageIcon, Upload } from 'lucide-react';
...
const CreateStartup = () => {
  ...
  const handleImageChange = (e) => {
    ...
    const validImages = files.filter(f => {
      const isImg = f.type.startsWith('image/') || /\.(jpg|jpeg|png|gif|webp|heic|heif)$/i.test(f.name);
      return isImg;
    });
    
    if (validImages.length < files.length) {
      toast.error('Only image files (.jpg, .png, etc.) are allowed in the gallery.');
    }
  ...
  const handleDocChange = (e) => {
    ...
    if (file) {
      ...
      // If it's a known image type but user tries to put it in doc field
      const isActuallyImg = file.type.startsWith('image/') || /\.(jpg|jpeg|png|gif|webp|heic|heif)$/i.test(file.name);
      
      if (!isDoc || (isActuallyImg && !file.name.toLowerCase().endsWith('.pdf'))) {
        toast.error('Please upload a PDF or PowerPoint file for verification.');
        e.target.value = '';
        return;
      }
    }
  ...
  const canProceed = () => {
    ...
    if(step===3) return user?.isEmailVerified && verificationDoc;
    return true;
  };

  const handleSubmit = async () => {
    if (!verificationDoc) return toast.error('Verification Document is required.');
    ...
    try {
      ...
      fd.append('fundingGoal',form.fundingGoal); 
      fd.append('valuation',form.valuation);
      fd.append('equity',form.equity); 
      ...
      fd.append('verificationDocument', verificationDoc);

      console.log('Submitting Startup with files:', {
        logo: logo?.name,
        images: images.map(i => i.name),
        doc: verificationDoc?.name
      });

      await api.post('/startups', fd, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      
      toast.success('Startup submitted! Pending admin approval.');
      navigate('/dashboard');
    } catch(err) { 
      console.error(err);
      toast.error(err.response?.data?.error || 'Failed to create startup'); 
    }
  ...
  return (
    <div style={{maxWidth:'640px',margin:'0 auto', paddingBottom: '4rem'}}>
      <div style={{marginBottom:'2.5rem'}}>
        <h1 style={{fontSize:'2rem',fontWeight:900,marginBottom:'0.5rem',display:'flex',alignItems:'center',gap:'0.75rem', letterSpacing: '-0.02em'}}><Rocket size={32} color="#6366f1"/> Create Startup</h1>
        <p style={{color:'var(--text-secondary)'}}>Launch your vision and connect with verified investors.</p>
      </div>
        ...
        {STEPS.map((s,i)=>(
          <React.Fragment key={i}>
            <div style={{display:'flex',flexDirection:'column',alignItems:'center',flex: 1}}>
              <div style={{width:'36px',height:'36px',borderRadius:'50%',background:i<step?'#10b981':i===step?'#6366f1':'var(--bg-tertiary)',display:'flex',alignItems:'center',justifyContent:'center', transition: 'all 0.3s', boxShadow: i===step ? '0 0 15px rgba(99,102,241,0.3)' : 'none'}}>
      ...
      <div className="card" style={{padding:'2.5rem', borderRadius: '24px', boxShadow: '0 10px 30px rgba(0,0,0,0.02)'}}>
        {step===0&&(
            ...
            <div style={{ border: '1px dashed var(--border)', padding: '1.5rem', borderRadius: '16px', background: 'var(--bg-secondary)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <label htmlFor="logo-upload" className="btn-secondary" style={{ padding: '0.5rem 1rem', fontSize: '0.8rem', cursor: 'pointer' }}>
                    <Upload size={14} /> {logo ? 'Change Logo' : 'Choose Logo'}
                  </label>
                  {logo && <button onClick={() => setLogo(null)} style={{ marginLeft: '1rem', background: 'none', border: 'none', color: '#f43f5e', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer' }}>Remove</button>}
                </div>
              </div>
            </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.5rem' }}>
                {form.teamMembers.map((m,i)=>(
                  <div key={i} style={{display:'flex',alignItems:'center',gap:'1rem',padding:'1rem',background:'var(--bg-secondary)',borderRadius:'16px',border:'1px solid var(--border)'}}>
                    <div style={{width:'40px',height:'40px',borderRadius:'12px',background:'var(--bg-card)',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:800,color:'#6366f1'}}>{m.name[0]}</div>
        ...
        {step===3&&(
          <div style={{display:'flex',flexDirection:'column',gap:'1.5rem'}}>
            <h2 style={{fontSize:'1.25rem',fontWeight:800}}>Review & Submit</h2>
            ...
            <div style={{padding:'1.25rem',background:'rgba(245,158,11,0.08)',border:'1px solid rgba(245,158,11,0.2)',borderRadius:'16px',fontSize:'0.9rem',color:'#f59e0b', display: 'flex', gap: '0.75rem'}}>
              <FileText size={20} style={{ flexShrink: 0 }} />
              <div>Please upload a Verification Document. This is only visible to admins for security review.</div>
            </div>
            
            <div>
              {lbl('Verification Document (PDF/PPTX) *')}
              <div style={{ position: 'relative' }}>
                ...
                />
                {verificationDoc && (
                  <div style={{ marginTop: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#10b981', fontSize: '0.85rem', fontWeight: 700 }}>
                    <Check size={16} /> Selected: {verificationDoc.name}
                  </div>
                )}
              </div>
            </div>
            ...
            {!user?.isEmailVerified && (
              <div style={{padding:'1.25rem',background:'rgba(244,63,94,0.08)',border:'1px solid rgba(244,63,94,0.2)',borderRadius:'16px',fontSize:'0.9rem',color:'#f43f5e', display: 'flex', gap: '0.75rem'}}>
                <X size={20} style={{ flexShrink: 0 }} />
                <div>You must verify your Email in <Link to="/settings" style={{color: '#f43f5e', textDecoration: 'underline', fontWeight: 800}}>Settings</Link> to submit.</div>
              </div>
            )}
          </div>
        )}
        ...
        <div style={{display:'flex',justifyContent:'space-between',marginTop:'2.5rem',paddingTop:'1.5rem',borderTop:'1px solid var(--border)'}}>
          <button onClick={()=>setStep(p=>p-1)} className="btn-secondary" disabled={step===0} style={{ padding: '0.65rem 1.5rem' }}><ChevronLeft size={18}/> Back</button>
          {step<STEPS.length-1
            ? <button onClick={()=>setStep(p=>p+1)} className="btn-primary" disabled={!canProceed()} style={{ padding: '0.65rem 1.5rem' }}>Next <ChevronRight size={18}/></button>
            : <button onClick={handleSubmit} className="btn-primary" disabled={loading || !canProceed()} style={{ padding: '0.65rem 2rem', boxShadow: '0 8px 25px rgba(99,102,241,0.3)' }}>{loading?'Submitting...':<><Rocket size={18}/> Submit Startup</>}</button>
          }
        </div>
      </div>
    </div>
